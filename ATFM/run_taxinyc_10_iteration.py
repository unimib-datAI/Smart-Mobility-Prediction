    ...
    for iteration in range(10):
        ...
        for epoch in range(tconf.max_epoch):
            ...
            if save_perform <= best_test_err:
                ...
                torch.save(model.state_dict(), log_dir+f'seed_{iteration*18}.model')
            else:
                early_stopping += 1
            if early_stopping > 50 or epoch == (tconf.max_epoch-1):
                torch.save(model.state_dict(), log_dir+f'seed_{iteration*18}.model')
                break
        ...
        model.load_state_dict(torch.load(log_dir+f'seed_{iteration*18}.model'))
        model.eval()
        with torch.no_grad():
            for i, (X, X_ext, Y, Y_ext) in enumerate(test_loader, 0):
                X = X.cuda()
        ...
        # save to csv
        csv_name = os.path.join('results', f'ATFN_taxinyc.csv')
