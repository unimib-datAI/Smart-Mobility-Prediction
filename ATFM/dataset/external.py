...
from .utils import timestamp2array, timestamp2vec_origin, transtr, transtrlong, transtr24, transtr23
...
class ExtDat:
    def __init__(self, datapath, dataset='TaxiBJ'):
        ...
        f = h5py.File(fname, 'r')
        timeslots = np.array(f['date'])
        wind_speed = np.array(f['WindSpeed'])
        ...
        if fourty_eight:
            ...
            try:
                ...
                return np.array(
                    [
                        self.meteor_data[
                            self.M[transtr23(ts-np.timedelta64(60, 'm') if previous_meteorol else ts)]
                        ] for ts in timestamp_list
                    ]
                )
