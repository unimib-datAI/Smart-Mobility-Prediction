...
class Dataset:
    ...
    def __init__(self, dconf, test_days=-1, datapath=datapath):
        ...
        if self.dataset == 'TaxiBJ':
            ...
            self.m_factor = 1 #math.sqrt(1. * 16 * 8 / 81)
            self.ext_dim = 33 if dconf.ext_time_flag else 8
        
        elif self.dataset == 'TaxiNYC':
            self.datafolder = 'TaxiNYC'
            self.dataname = [
                'NYC10_Taxi_M16x8_T60_InOut.h5',
                'NYC11_Taxi_M16x8_T60_InOut.h5',
                'NYC12_Taxi_M16x8_T60_InOut.h5',
                'NYC13_Taxi_M16x8_T60_InOut.h5',
                'NYC14_Taxi_M16x8_T60_InOut.h5'
                ]
            self.nb_flow = 2
            self.dim_h = 16
            self.dim_w = 8
        ...
        elif self.dataset == 'TaxiNYC_loro':
            self.datafolder = 'TaxiNYC_loro'
    ...
    def get_raw_data(self):
        ...
        for filename in self.dataname:
            ...
            _raw_data = np.array(f['data'])
            _raw_ts = np.array(f['date'])
            f.close()
