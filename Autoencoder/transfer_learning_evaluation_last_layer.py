...
# save to csv
csv_name = os.path.join('results', f'TL_taxiBJ_roma32x32_training_nolast_layer_results.csv')
save_to_csv(score, csv_name)

# save real vs predicted
fname = 'model3_RomaNord32x32_trained_no_lastlayer.h5'
h5 = h5py.File(fname, 'w')
h5.create_dataset('Y_real', data=Y_test)
h5.create_dataset('Y_pred', data=Y_pred)
h5.create_dataset('timestamps', data=timestamp_test)
...
# save to csv
csv_name = os.path.join('results', f'TL_taxiNY_roma16x8_training_nolast_layer_results.csv')
save_to_csv(score, csv_name)

# save real vs predicted
fname = 'model3_RomaNord16x8_trained_no_lastlayer.h5'
h5 = h5py.File(fname, 'w')
h5.create_dataset('Y_real', data=Y_test)
h5.create_dataset('Y_pred', data=Y_pred)
h5.create_dataset('timestamps', data=timestamp_test)
