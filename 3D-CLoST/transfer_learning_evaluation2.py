...
#nb_epoch = 100
#batch_size = 16
#hyperparams_name = 'TaxiBJ_Rome'add_half
#fname_param = os.path.join('MODEL', '{}.best.h5'.format(hyperparams_name))
#model_checkpoint = ModelCheckpoint(
#        fname_param, monitor='val_rmse', verbose=0, save_best_only=True, mode='min')
#history = model.fit(X_train, Y_train,
...
nb_epoch = 100
batch_size = 16
hyperparams_name = '3dclost_roma16x8_2'
fname_param = os.path.join('MODEL', '{}.best.h5'.format(hyperparams_name))
model_checkpoint = ModelCheckpoint(
        fname_param, monitor='val_rmse', verbose=0, save_best_only=True, mode='min')
history = model.fit(X_train, Y_train,
...
# save to csv
csv_name = os.path.join('results', f'roma16x8_results_2.csv')
save_to_csv(score, csv_name)
...
# save real vs predicted
fname = '3dclost_RomaNord16x8_2.h5'
h5 = h5py.File(fname, 'w')
h5.create_dataset('Y_real', data=Y_test)
h5.create_dataset('Y_pred', data=Y_pred)
h5.create_dataset('timestamps', data=timestamp_test)
...
# save to csv
csv_name = os.path.join('results', f'TL_taxiNY_roma16x8_training_results_2.csv')
save_to_csv(score, csv_name)

# save real vs predicted
fname = '3dclost_RomaNord16x8_trained_2.h5'
h5 = h5py.File(fname, 'w')
h5.create_dataset('Y_real', data=Y_test)
h5.create_dataset('Y_pred', data=Y_pred)
h5.create_dataset('timestamps', data=timestamp_test)
