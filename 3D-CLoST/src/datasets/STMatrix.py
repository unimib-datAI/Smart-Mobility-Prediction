...
class STMatrix(object):
    ...
    def __init__(self, data, timestamps, T=48, CheckComplete=True, Hours0_23=False, add_half=False):
    ...
    def create_dataset(self, len_closeness=3, len_trend=3, TrendInterval=7, len_period=3, PeriodInterval=1):
        ...
        #add half an hour before and after
        if (len_period > 0 and self.add_half):
            depends[1] = [depends[1][0]-1] + depends[1] + [depends[1][-1]+1]
        if (len_trend > 0 and self.add_half):
            depends[2] = [depends[2][0]-1] + depends[2] + [depends[2][-1]+1]
